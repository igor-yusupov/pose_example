...
class ImageHelper(object):
    ...
    def bgr2gray(img, keepdim=False):
        ...
        Args:
            ...
            keepdim (bool): If False (by default), then return the grayscale
                image with 2 dims, otherwise 3 dims.
    ...
    def resize(img, target_size, interpolation=None):
        ...
        if isinstance(img, Image.Image):
            return img.resize(
                tuple(target_size), PIL_INTER_DICT[interpolation])

        elif isinstance(img, np.ndarray):
            return cv2.resize(
                img, tuple(target_size),
                interpolation=CV2_INTER_DICT[interpolation])
    ...
    def fig2img(fig):
        """
        @brief Convert a Matplotlib figure to a PIL Image in RGBA format and
         return it
    ...
    def is_img(img_name):
        ...
        return any(
            img_name.endswith(extension) for extension in IMG_EXTENSIONS)

    @staticmethod
    def imgpath(data_dir, image_name):
        match_list = [os.path.join(data_dir, '{}{}'.format(image_name, ext))
                      for ext in IMG_EXTENSIONS
                      if os.path.exists(
                        os.path.join(data_dir, '{}{}'.format(
                            image_name, ext)))]
